<?php
 // patient summary display for corresponding doctor
session_start();
...
<script>
	...
	// search the patient wrt patient id and patient name.
	function searchpat()
	{
...
/*	$host='192.168.108.40';
	...
	*/
	...
	while($result=mysql_fetch_array($query))
	{
		...
		$q1=mysql_query("SELECT hr,br,bp_systole,bp_diastole,ecg,spo2,abnormal FROM summary WHERE pid='$resid' ORDER BY time DESC") or die(mysql_error());
		$one2=0;
		while($row=mysql_fetch_array($q1))
		{
			...
			{
				if($row['abnormal']=='1')
					echo "<tr bgcolor='#FF0000' style='color:white;'>";
				...
				echo "<td>".$row['ecg']."</td>";
				echo "<td>".$row['spo2']."</td>";?>
				<td><button type="button" class="mybutton" onclick="getRow(this)" style="color:black;">View Profile</button></td>
			<?php
			}
			$one2++;
		}
